...
@Composable
fun TransitionGrid(modifier: Modifier = Modifier) {
  val animIn = listOf(
  "fadeIn" to fadeIn(),
  "slideIn" to slideIn { fullSize -> IntOffset(fullSize.width, fullSize.height) },
  ...
  val animOut = listOf(
  "fadeOut" to fadeOut(),
  "slideOut" to slideOut { fullSize -> IntOffset(fullSize.width, fullSize.height) },
      ...
      Column(Modifier.weight(1f)) {
        animIn.forEach { (title, transition) ->
          TransitionView(
            ...
            enter = transition, exit = ExitTransition.None
          )
        }
      }
      Column(Modifier.weight(1f)) {
        animOut.forEach { (title, transition) ->
          TransitionView(
            show = !show, title = title,
            enter = EnterTransition.None, exit = transition
          )
        }
      }
    }
